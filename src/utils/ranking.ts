...
/**
 * Hybrid ranking: featured events first, then out-of-town events sink, then by
 * quality signal, then soonest. Pure and side-effect free so it can be reused
 * identically once events come from the real backend instead of the mock
 * source.
 */
export function rankEvents(events: readonly StockholmEvent[]): StockholmEvent[] {
  return [...events].sort((a, b) => {
    if (a.isFeatured !== b.isFeatured) return a.isFeatured ? -1 : 1;
    const aAway = isOutOfTown(a);
    const bAway = isOutOfTown(b);
